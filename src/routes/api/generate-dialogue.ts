...
انطق النص كما هو حرفياً دون أي مقدمات أو أسماء متحدثين، ودون حذف أو اختصار أو إضافة.`;
  ...
  const roleLine =
    role === "host"
      ...
      : `أنت المحصّل: شاب سعودي في العشرينات، تتكلم بلهجة سعودية عامية بسيطة 100٪، وأداؤك واقعي جداً وكأنك واقف قدّام الشخص وجهاً لوجه، مو قارئ نص ولا سارد قصة.

- نوّع نبراتك بشكل واضح وحيّ حسب معنى الجملة: نبرة جدية حازمة، نبرة غاضبة، نبرة تهديد وضغط، نبرة مرتفعة وحاسمة في المواقف القوية، ونبرة أهدأ وأخفض عند ذكر التفاصيل والمعلومات.
- ارفع الصوت وحدّته (Volume + Pitch) في الجُمل التي فيها تحذير أو تهديد أو قسم أو تأكيد قوي، وشدّد على الكلمات المفتاحية مثل: «أُقسم بالله»، «والله العظيم»، «التوقيف»، «القبض»، «السجن»، «الترحيل»، «قرارك»، «صادق معاي»، «لا تلف ولا تدور».
...
- استخدم وقفات قصيرة حادة قبل الجُمل المفصلية عشان تشدّ انتباه السامع، وتنفّس عفوي بين المقاطع.
...
async function synthesizeSpeechPcm(params: {
  ...
  const response = await fetch("https://ai.gateway.lovable.dev/v1/audio/speech", {
    ...
    body: JSON.stringify({
      ...
      input: params.text,
      // الأصوات ثابتة إلزامياً: المذيع رجالي ناضج (onyx)، والمحصّل شاب سعودي في العشرينات (echo)
      voice: params.role === "host" ? "onyx" : "echo",
